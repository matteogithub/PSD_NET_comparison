...
ns=round(T*fs);
nt=10; %trials
f0 = [6 10]; % theta frequencies
df = 1/T;
data=zeros(nc,ns);
PRtot=linspace(1,4,100); % ratio of power of second frequency wrt first one
...
%%
figure;scatter(1./PRtot,AEC_av);xlabel('PSD(theta/alpha ratio)');ylabel('|AEC alpha|');set(gca,'FontSize',14)
title(['Pearson correlation = ' num2str(corr2(PRtot,AEC_av))])
figure;scatter(1./PRtot,AEC_no_av);xlabel('PSD(theta/alpha ratio)');ylabel('|AEC alpha no orth|');set(gca,'FontSize',14)
